...
class SearchMetadata(BaseModel):
    ...
    sources: list[str] = Field(default_factory=list, description="List of providers that returned data.")
...
class DrugInfo(BaseModel):
    ...
    warnings: list[str] = Field(default_factory=list, description="Warnings associated with the drug.")
...
class DrugExplanation(BaseModel):
    drug_info: DrugInfo | None = Field(None, description="FDA information about the drug.")
    papers: list[ResearchPaper] = Field(default_factory=list, description="Recent research papers related to the drug.")
    trials: list[ClinicalTrial] = Field(default_factory=list, description="Active clinical trials for the drug.")


class SearchResults(BaseModel):
    drugs: list[DrugInfo] = Field(default_factory=list, description="Drugs matching the query.")
    papers: list[ResearchPaper] = Field(default_factory=list, description="Research papers matching the query.")
    trials: list[ClinicalTrial] = Field(default_factory=list, description="Clinical trials matching the query.")
    metadata: SearchMetadata | None = Field(None, description="Metadata about the query execution.")
...
class ConditionSummary(BaseModel):
    ...
    papers: list[ResearchPaper] = Field(default_factory=list, description="Recent research highlights.")
    trials: list[ClinicalTrial] = Field(default_factory=list, description="Key recruiting clinical trials.")
