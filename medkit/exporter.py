...
class Exporter:
    ...
    def to_csv(data: Any, path: str | Path):
        ...
        if hasattr(data, "drugs"): # SearchResults
            for d in data.drugs: records.append({"type": "drug", "name": d.brand_name, "detail": d.generic_name})
            for p in data.papers: records.append({"type": "paper", "name": p.title, "detail": p.pmid})
            for t in data.trials: records.append({"type": "trial", "name": t.nct_id, "detail": t.status})
