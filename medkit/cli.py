...
from medkit import MedKit, MedKitError, ResearchPaper, ClinicalTrial, SearchResults, MedicalGraph, ConditionSummary
    ...
    with MedKit() as med:
        ...
        for name, provider in med._providers.items():
            ...
            try:
                ...
                status_str = "[bold green]ONLINE[/bold green]" if healthy else "[bold red]OFFLINE[/bold red]"
...
def interactions(drugs: list[str]):
    ...
    with MedKit() as med:
        ...
        if not warns:
            console.print(f"[bold green]No known interactions found for: {', '.join(drugs)}[/bold green]")
            return
...
@app.command()
def papers(query: str, limit: int = 5, as_json: bool = False, links: bool = False, open: bool = False):
    """
...
@app.command()
def trials(condition: str, limit: int = 5, as_json: bool = False, links: bool = False, open: bool = False):
    """
...
def search(query: str, as_json: bool = False):
    ...
    try:
        with MedKit() as med:
            with console.status(f"[bold blue]Searching for '{query}' across all sources...[/bold blue]"):
...
def summary(query: str):
    ...
    try:
        with MedKit() as med:
            with console.status(f"[bold blue]Generating summary for '{query}'...[/bold blue]"):
                s = med.summary(query)
...
def explain(name: str):
    ...
    try:
        with MedKit() as med:
            with console.status(f"[bold blue]Fetching comprehensive data for {name}...[/bold blue]"):
...
def ask(question: str):
    ...
    try:
        with MedKit() as med:
            with console.status(f"[bold blue]Processing question: '{question}'...[/bold blue]"):
...
def _render_summary(s: ConditionSummary):
    ...
    if s.drugs:
        ...
        for d in s.drugs: console.print(f"- {d}")
    if s.papers:
        console.print(f"\n[bold cyan]Latest Research:[/bold cyan]")
        for p in s.papers[:5]: console.print(f"- {p.title} ({p.year})")
    if s.trials:
        console.print(f"\n[bold magenta]Clinical Trials:[/bold magenta]")
        for t in s.trials: console.print(f"- {t.nct_id}: {t.status} - {t.title}")
    else:
        console.print("\n[bold magenta]Clinical Trials:[/bold magenta]")
        console.print("No active recruiting trials found")
...
def _render_papers(papers: list[ResearchPaper]):
    console.print("\n[bold cyan]=== Latest Research ===[/bold cyan]")
    for p in papers: console.print(f"- {p.title} ({p.year})")


def _render_trials(trials: list[ClinicalTrial]):
    console.print("\n[bold magenta]=== Clinical Trials ===[/bold magenta]")
    for t in trials: console.print(f"- {t.nct_id}: {t.status} - {t.title}")
...
def graph(query: str):
    ...
    try:
        with MedKit() as med:
            with console.status(f"[bold blue]Building relationship graph for '{query}'...[/bold blue]"):
                g = med.graph(query)
