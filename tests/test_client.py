...
from medkit.client import MedKit, AsyncMedKit
from medkit.models import DrugInfo, ResearchPaper, ClinicalTrial
...
def test_papers_search(mock_medkit):
    mock_paper = ResearchPaper(
        pmid="12345", title="Test Paper", authors=[], journal="Test Journal", year=2023, abstract=""
    )
...
def test_trials_search(mock_medkit):
    mock_trial = ClinicalTrial(
        nct_id="NCT123", title="Test Trial", status="RECRUITING", phase=[], location=[], eligibility=""
    )
...
def test_explain_drug(mock_medkit):
    mock_drug = DrugInfo(brand_name="TestDrug", generic_name="Test", warnings=[], manufacturer="TestMakers")
    mock_medkit._providers["openfda"].search_sync.return_value = mock_drug
...
async def test_async_search():
    async with AsyncMedKit() as med:
        ...
        mock_drug = DrugInfo(brand_name="Aspirin", generic_name="A", warnings=[], manufacturer="B")
